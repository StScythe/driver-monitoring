...
**Пояснение к снижению 0% для зевка:** в данном тестовом сеансе был зафиксирован **один зевок длительностью 1.80 секунды** (54 кадра). Окно политопа для зевка составляет 15 кадров (0.5 секунды). Поскольку зевок превышал минимальную длительность, **оба метода** (покадровый и политопный) успешно его обнаружили. Снижение 0% в данном случае означает, что не было ложных кратковременных срабатываний по зевку — все покадровые события соответствовали истинному зевку и были достаточно длительными для подтверждения политопом.

Для сонливости, отвлечения и наклона политопный подход обеспечил снижение ложных срабатываний на **97-99%**, отсекая моргания (0.1-0.4 сек) и быстрые повороты головы (< 2 сек).

**Выводы:**
- Политопная фильтрация эффективно отсекает кратковременные события (моргания, быстрые повороты)
- Истинные длительные события (зевок 1.8 с, сонливость 4.8 с) успешно детектируются — снижение 0% для зевка это подтверждает
...
## 6. Выявленные технические ограничения MVP
В ходе разработки и тестирования MVP выявлены следующие ограничения, связанные с инженерной реализацией:

| Ограничение | Описание | Влияние на систему |
|:---|:---|:---|
| **Зависимость от USB-камеры** | Система тестировалась с USB-веб-камерами стандартного качества. При использовании низкокачественных камер (разрешение ниже 480p, низкая частота кадров) детекция лица становится нестабильной | Рекомендуемое разрешение — не менее 720p при 30 FPS |
| **Однопоточная обработка** | В текущей версии все вычисления выполняются в одном потоке. При 15 FPS загрузка CPU составляет 45-55%, что оставляет запас, но не позволяет параллельно запускать дополнительные модули (например, детекцию телефона или ремня безопасности) | Для расширения функциональности потребуется переход на многопоточную архитектуру |
| **Отсутствие GPU-ускорения** | MVP работает на CPU (Intel Core i5). MediaPipe может использовать GPU, но в текущей реализации эта опция отключена для обеспечения совместимости с разными системами | На целевой платформе Jetson Nano (128 ядер CUDA) потребуется адаптация и тестирование GPU-ускорения |
| **Фиксированная частота кадров** | Система ожидает 30 FPS от камеры. Если камера выдает 25 или 15 FPS, окна политопа (15 и 60 кадров) не соответствуют заданной длительности в секундах | Требуется динамическое определение FPS камеры и автоматическая корректировка размеров окон |
| **Отсутствие сохранения сессий при аварийном завершении** | При внезапном закрытии окна (например, по ошибке пользователя) или отключении камеры текущая сессия мониторинга не сохраняется. Данные за несохраненный период теряются | Требуется реализация автоматического сохранения состояния (checkpointing) с интервалом 1 секунда |
| **Калибровка требует идеальных условий** | Калибровка предполагает, что водитель 30 секунд сидит с открытыми глазами и закрытым ртом. В реальных условиях водитель может моргать, разговаривать или отворачиваться | В производственной версии требуется более гибкая калибровка с возможностью прерывания и возобновления |
